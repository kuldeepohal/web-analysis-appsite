...
export function getRazorpayKeyId() { return credentials().keyId; }
...
export async function fetchRazorpayOrder(orderId) {
  const response = await fetch(`${RAZORPAY_API}/orders/${encodeURIComponent(orderId)}`, { headers: { Authorization: authHeader() }, cache: 'no-store' });
  const data = await response.json();
